# Genera un informe HTML standalone (mapa + tabla) de peores contenedores por
# Condicion ("Basura Afuera" y/o "Requiere Limpieza"), a partir del mismo cálculo
# de PeoresCondicion.R. Ver documentacion_analisis_peores_condicion.md para el
...
resultado <- resumen_confiable %>%
  mutate(
    ...
    en_riesgo = condicion_alta  # mismo criterio que "cuadrante != OK" en PeoresCondicion.R
  ) %>%
...
mapa_data <- resultado %>%
  transmute(
    ...
    n_requiere_limpieza,
    n_alguna_condicion,
    tasa_condicion = round(tasa_condicion, 4),
...
saveRDS(list(json_mapa = json_mapa, json_tabla = json_tabla,
             ...
             mediana_condicion = mediana_condicion, mediana_frecuencia = mediana_frecuencia),
        "informes/Condicion_peorescasos/datos_informe_html.rds")
